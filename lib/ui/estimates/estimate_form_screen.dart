...
class EstimateFormScreen extends StatefulWidget {
  final EstimateApiService estimateService;
...
class _EstimateFormScreenState extends State<EstimateFormScreen> {
  ...
  Future<void> _loadFormData() async {
    try {
      final results = await Future.wait([
        ...
        widget.estimateService.getNextNumber(widget.token, companyId: widget.companyId),
      ]);
      _customers = (results[0] as CustomerListResponse).customers;
      _catalogItems = (results[1] as ItemListResponse).items;
      _estimateNumber = results[2] as String;
  ...
  Future<void> _save() async {
    ...
    try {
      final payload = {
        'estimate_date': DateFormat('yyyy-MM-dd').format(_estimateDate),
      ...
      await widget.estimateService.createEstimate(widget.token, payload, companyId: widget.companyId);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Estimate created successfully')));
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: InkWell(
                    onTap: () => _pickDate(_estimateDate, (d) => setState(() => _estimateDate = d)),
                    child: InputDecorator(
                      decoration: const InputDecoration(labelText: 'Estimate Date *'),
                      child: Text(DateFormat('yyyy-MM-dd').format(_estimateDate)),
                    ),
                  ),
                ),
                ...
                Expanded(
                  child: InkWell(
                    onTap: () => _pickDate(_expiryDate ?? DateTime.now().add(const Duration(days: 30)), (d) => setState(() => _expiryDate = d)),
                    child: InputDecorator(
                      decoration: const InputDecoration(labelText: 'Expiry Date'),
                      child: Text(_expiryDate != null ? DateFormat('yyyy-MM-dd').format(_expiryDate!) : 'Select'),
                    ),
                  ),
                ),
              ],
