...
void main() {
  ...
  for (final file in files) {
    ...
    content = content.replaceAll(RegExp(r"\(\s*json\[\s*'(.*?)'\s*\]\s*\?\?\s*0\s*\)\s*/\s*100\.0"), r"parseAmount(json['\1'])");
    content = content.replaceAll(RegExp(r"\(\(\s*json\[\s*'(.*?)'\s*\]\s*\?\?\s*0\s*\)\s*as\s*num\)\.toDouble\(\)\s*/\s*100\.0"), r"parseAmount(json['\1'])");
    content = content.replaceAll(RegExp(r"\(\s*json\[\s*'(.*?)'\s*\]\s*\?\?\s*0\s*\)\.toDouble\(\)"), r"parseDouble(json['\1'])");
    content = content.replaceAll(RegExp(r"\(\s*inv\[\s*'(.*?)'\s*\]\s*\?\?\s*0\s*\)\s*/\s*100\.0"), r"parseAmount(inv['\1'])");
